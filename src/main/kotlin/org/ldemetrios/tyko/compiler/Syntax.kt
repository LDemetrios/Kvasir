...
import org.ldemetrios.tyko.ffi.intoKotlin
...
fun TypstSharedLibrary.parseSource(string: String, mode: SyntaxMode): FlattenedSyntaxTree {
    ...
    val flattened = parse_syntax(thick, mode.ordinal)
    flattened.read()
    thick.close()
    val list = flattened.intoKotlin(string)
    release_flattened_tree(flattened)
    return FlattenedSyntaxTree(list)
}
